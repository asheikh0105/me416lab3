...
class RobotController:
    def __init__(self, robot_id: str, ip_suffix: str, cfg: Config):
        self.robot_id = robot_id  # e.g., "limo777"
        self.ip_suffix = ip_suffix  # e.g., "101"
        self.cfg = cfg
        ...
        print(f"  Robot ID (MQTT): {robot_id}")
        print(f"  IP Suffix (TCP): {ip_suffix}")
    ...
    def setup_tcp(self):
        ...
        limo_ip = self.cfg.limoIpPrefix + self.ip_suffix
        ...
        while self.running:
            ...
            if loop_count % 30 == 0:
                print(f"[{self.sim_time:.1f}s] Pos:({self.my_robot.pose[0]:.2f},{self.my_robot.pose[1]:.2f}) "
                      f"Goal:{dist_to_goal:.2f}m V:{lin_vel:.2f} W:{np.rad2deg(ang_vel):.1f}° "
...
def main():
    ...
    # Get robot ID for MQTT topics
    robot_id = input("Enter LIMO ID for MQTT topics (e.g., limo777): ").strip()
    
    if not robot_id:
        print("Error: Robot ID cannot be empty")
        return
    
    # Get IP address suffix
    ip_suffix = input("Enter IP address last 3 digits (e.g., 101): ").strip()
    
    if not ip_suffix:
        print("Error: IP suffix cannot be empty")
        return
    
    print(f"\nInitializing controller...")
    print(f"  MQTT Topics: rb/{robot_id}, goal/{robot_id}, cmd/{robot_id}")
    print(f"  TCP Address: 192.168.1.{ip_suffix}:12345")
    
    # Create configuration
    cfg = Config()
    
    # Create and run controller
    controller = RobotController(robot_id, ip_suffix, cfg)
    controller.run()
